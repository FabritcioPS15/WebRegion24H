...
export default function Header() {
  ...
  return (
    <>
      {/* Real Fixed Header */}
      <header className="bg-white border-b border-gray-200 fixed top-0 left-0 w-full z-[60] transition-all duration-500 shadow-sm">
        {/* Top Bar */}
        <div className={`bg-accent text-white overflow-hidden transition-all duration-500 text-[10px] font-black tracking-[0.2em] text-center uppercase ${isScrolled ? 'h-0 py-0' : 'h-auto py-1.5'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          {/* Masthead */}
          <div className={`border-b border-gray-100 flex flex-col items-center transition-all duration-700 ${isScrolled ? 'py-2 md:py-2' : 'py-6 md:py-10'}`}>
            <div className={`w-full flex justify-between items-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest font-sans transition-all duration-500 ${isScrolled ? 'h-0 opacity-0 mb-0 overflow-hidden' : 'h-auto opacity-100 mb-6 md:mb-8'}`}>
            ...
            <Link to="/" className="text-center group">
              ...
              >
                <h1 className={`font-serif font-black text-accent tracking-tighter leading-none transition-all duration-700 ${isScrolled ? 'text-2xl md:text-3xl' : 'text-4xl sm:text-6xl lg:text-8xl'}`}>
                  REGIÓN <span className="text-brand">24H</span>
                </h1>
                <div className={`h-0.5 bg-brand transition-all duration-700 ${isScrolled ? 'w-full mt-1' : 'w-1/4 mx-auto mt-2 group-hover:w-full'}`} />
                <p className={`font-serif italic text-gray-400 tracking-widest transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 opacity-0' : 'h-auto opacity-100 text-xs md:text-sm mt-4'}`}>FUNDADO PARA LA EXCELENCIA INFORMATIVA</p>
              </motion.div>
            </Link>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <ul className={`hidden lg:flex items-center w-full ${mobileMenuOpen ? 'flex' : ''} flex-col lg:flex-row justify-center`}>
                ...
                <li className="flex items-center px-4">
                  <Search className="h-4 w-4 text-accent cursor-pointer hover:text-brand" />
                </li>
              </ul>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className={`border-b border-gray-100 flex flex-col items-center transition-all duration-700 ${isScrolled ? 'py-2 md:py-2' : 'py-6 md:py-10'}`}>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className={`h-0.5 bg-brand transition-all duration-700 ${isScrolled ? 'w-full mt-1' : 'w-1/4 mx-auto mt-2'}`} />
              <p className={`font-serif italic text-gray-400 tracking-widest transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 opacity-0' : 'h-auto opacity-100 text-xs md:text-sm mt-4'}`}>FUNDADO PARA LA EXCELENCIA INFORMATIVA</p>
            </div>
          </div>
