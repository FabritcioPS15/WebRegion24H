...
export default function VideosSection() {
  const { displayVideos: videos, changedIds, isPreviewMode, isLoading } = useNews();
  ...
  const visibleVideos = videos.slice(startIndex, startIndex + 3);
  ...
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex items-center gap-3">
            <Link href="/videos" className="text-xs font-black text-brand uppercase tracking-widest hover:text-accent transition-colors flex items-center gap-2 group mr-4">
              Ver todos
            ...
            {videos.length > 3 && (
              <div className="flex gap-2">
                <button 
                  ...
                  className={`p-2 border ${startIndex === 0 ? 'border-gray-100 text-gray-300' : 'border-accent text-accent hover:bg-brand hover:text-white hover:border-brand'} transition-all`}
                >
                  <ChevronLeft size={16} />
                </button>
                <button 
                  ...
                  className={`p-2 border ${startIndex + 3 >= videos.length ? 'border-gray-100 text-gray-300' : 'border-accent text-accent hover:bg-brand hover:text-white hover:border-brand'} transition-all`}
                >
